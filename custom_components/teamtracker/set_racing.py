...
async def async_set_racing_values(
    ...
    if competition is None or competitor is None or opponent is None:
        _LOGGER.debug(
            "%s: async_set_racing_values() 0: %s", sensor_name, sensor_name
        )
        return False
    ...
    race_key = str(new_values["league"]) + "-" + str(new_values["event_name"])
        ...
        new_values["last_play"] = (
            ...
            + str(
                await async_get_value(
                    competition, "competitors", x, "order", default=x
                )
            )
            + ". "
        )
