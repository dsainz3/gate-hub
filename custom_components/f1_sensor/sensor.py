...
from .const import DOMAIN, LATEST_TRACK_STATUS
...
async def async_setup_entry(
    ...
    mapping = {
        ...
        "track_weather": (
            ...
            data.get("weather_data_coordinator"),
        ),
        "race_lap_count": (
            F1RaceLapCountSensor,
            data.get("lap_count_coordinator"),
        ),
        ...
        "track_status": (
            F1TrackStatusSensor,
            data.get("track_status_coordinator"),
        ),
        "session_status": (
...
class F1NextRaceSensor(F1BaseEntity, SensorEntity):
    ...
    def __init__(
        self, coordinator, sensor_name, unique_id, entry_id, device_name
    ):
        super().__init__(
            coordinator, sensor_name, unique_id, entry_id, device_name
        )
    ...
    def state(self):
        ...
        if not next_race:
            return None
        return self.combine_date_time(
            next_race.get("date"), next_race.get("time")
        )
    ...
    def extra_state_attributes(self):
        ...
        timezone = self._timezone_from_location(
        ...
        sprint_start = self.combine_date_time(
            sprint.get("date"), sprint.get("time")
        )
        ...
        return {
            ...
            "first_practice_start_local": self._to_local(
                first_start, timezone
            ),
            "second_practice_start": second_start,
            "second_practice_start_local": self._to_local(
                second_start, timezone
            ),
            "third_practice_start": third_start,
            "third_practice_start_local": self._to_local(
                third_start, timezone
            ),
    ...
    async def _update_weather(self):
        ...
        if start_iso:
            ...
            same_day = [
                t
                for t in times
                if datetime.datetime.fromisoformat(t["time"]).date()
                == start_dt.date()
            ]
            if same_day:
                closest = min(
                ...
                instant_details = data_entry.get("instant", {}).get(
                    "details", {}
                )
                precip_1h = (
                ...
                race_symbol = forecast_block.get("summary", {}).get(
                    "symbol_code"
                )
                race_icon = SYMBOL_CODE_TO_MDI.get(
                    race_symbol, self._attr_icon
                )
...
class F1LastRaceSensor(F1BaseEntity, SensorEntity):
    ...
    def state(self):
        ...
        winner = next(
            (r for r in results if r.get("positionText") == "1"), None
        )
    ...
    def extra_state_attributes(self):
        ...
        def _clean_result(r):
            return {
                ...
                "status": r.get("status"),
                "driver": {
                    "permanentNumber": r.get("Driver", {}).get(
                        "permanentNumber"
                    ),
                ...
                "constructor": {
                    "constructorId": r.get("Constructor", {}).get(
                        "constructorId"
                    ),
...
class F1RaceLapCountSensor(F1BaseEntity, RestoreEntity, SensorEntity):
    ...
    def _extract_current(self) -> dict | None:
        ...
        if isinstance(inner, dict) and (
            "CurrentLap" in inner
            or "TotalLaps" in inner
            or "LapCount" in inner
        ):
            return inner
        hist = getattr(self.coordinator, "data_list", None)
        if isinstance(hist, list) and hist:
    ...
    def _apply_payload(self, raw: dict) -> None:
        curr = self._to_int(
            raw.get("CurrentLap")
            if "CurrentLap" in raw
            else raw.get("LapCount")
        )
