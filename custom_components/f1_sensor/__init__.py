...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    race_coordinator = F1DataCoordinator(
        hass, API_URL, "F1 Race Data Coordinator"
    )
...
class WeatherDataCoordinator(DataUpdateCoordinator):
    ...
    def _has_timestamp(d: dict) -> bool:
        try:
            return any(
                k in d for k in ("Utc", "utc", "processedAt", "timestamp")
            )
        except Exception:
            return False
                ...
                async for payload in self._client.messages():
                    ...
                    for item in items:
                        ...
                        with contextlib.suppress(Exception):
                            if ts_raw and self._startup_cutoff:
                                ...
                                if ts < self._startup_cutoff:
                                    _LOGGER.debug(
                                        "RaceControl: Ignored old item at startup: %s",
                                        item,
                                    )
                                    continue
                        ...
                        if ident in self._seen_ids:
                            _LOGGER.debug(
                                "RaceControl: Duplicate suppressed: %s", ident
                            )
                            continue
    ...
    async def _async_update_data(self):
        try:
            ...
            while next_offset < total and safety < 50:
                ...
                praces = (pmr.get("RaceTable", {}) or {}).get(
                    "Races", []
                ) or []
                merge_page(praces)
                with contextlib.suppress(Exception):
            ...
            raise UpdateFailed(
                f"Error fetching season results: {err}"
            ) from err
                        ...
                        with contextlib.suppress(Exception):
                            if utc_str:
                                ...
                                if (
                                    ...
                                    and ts < self._startup_cutoff
                                ):
                                    _LOGGER.debug(
                                        "TrackStatus: Ignored old message: %s",
                                        msg,
                                    )
                                    continue
