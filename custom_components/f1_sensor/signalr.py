...
class SignalRClient:
    ...
    async def connect(self) -> None:
        ...
        _LOGGER.debug(
            "Subscribed to RaceControlMessages, TrackStatus, SessionStatus, WeatherData and LapCount"
        )
    ...
    async def _ensure_connection(self) -> None:
        ...
        while True:
            try:
                ...
                _LOGGER.warning(
                    "SignalR reconnect failed (%s). Retrying in %s s …",
                    err,
                    delay,
                )
                    ...
                    for hub_msg in payload["M"]:
                        if hub_msg.get("M") == "feed":
                            ...
                            if stream_name == "RaceControlMessages":
                                try:
                                    _LOGGER.debug(
                                        "Race control message: %s",
                                        hub_msg["A"][1],
                                    )
                                except Exception:
                                    _LOGGER.debug(
                                        "Race control message received (unparsed)"
                                    )
                            elif stream_name == "TrackStatus":
                                # Log TrackStatus updates at debug level similar to RaceControl
                                try:
                                    _LOGGER.debug(
                                        "Track status message: %s",
                                        hub_msg["A"][1],
                                    )
                                except Exception:
                                    _LOGGER.debug(
                                        "Track status message received (unparsed)"
                                    )
                            elif stream_name == "SessionStatus":
                                try:
                                    _LOGGER.debug(
                                        "Session status message: %s",
                                        hub_msg["A"][1],
                                    )
                                except Exception:
                                    _LOGGER.debug(
                                        "Session status message received (unparsed)"
                                    )
                            elif stream_name == "WeatherData":
                                try:
                                    _LOGGER.debug(
                                        "Weather data message: %s",
                                        hub_msg["A"][1],
                                    )
                                except Exception:
                                    _LOGGER.debug(
                                        "Weather data message received (unparsed)"
                                    )
                                ...
                                try:
                                    ...
                                    _LOGGER.debug(
                                        "Lap count message received (unparsed)"
                                    )
                elif "R" in payload:
