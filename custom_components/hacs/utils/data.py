...
class HacsData:
    ...
    def async_store_experimental_repository_data(
        ...
        if repository.data.installed:
            data["repository_manifest"] = (
                repository.repository_manifest.manifest
            )
            for key, default in EXPORTED_DOWNLOADED_REPOSITORY_DATA:
                if (
                    value := getattr(repository.data, key, default)
                ) != default:
                    data[key] = value
    ...
    async def restore(self):
        ...
        try:
            repositories = await async_load_from_store(
                self.hacs.hass, "repositories"
            )
        ...
        try:
            ...
            self.logger.critical(
                "<HacsData restore> [%s] Restore Failed!",
                exception,
                exc_info=exception,
            )
            return False
    ...
    def async_restore_repository(
        ...
        try:
            ...
            self.logger.warning(
                "<HacsData async_restore_repository> duplicate IDs %s",
                exception,
            )
            return
        ...
        if self.hacs.system.generator:
            repository.data.etag_releases = repository_data.get(
                "etag_releases"
            )
        ...
        if last_fetched := repository_data.get("last_fetched"):
            repository.data.last_fetched = datetime.fromtimestamp(
                last_fetched, UTC
            )
