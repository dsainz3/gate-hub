...
FORECAST_SENSOR_DESCRIPTIONS: tuple[
...
async def async_setup_entry(
    ...
    if coordinator.forecast_enable:
        sensors.extend(
            WundergroundPWSForecastSensor(
                coordinator, description, forecast_day=day
            )
...
class WundergroundPWSSensor(CoordinatorEntity, SensorEntity):
    ...
    def name(self):
        ...
        if (
            self.entity_description.key in self.coordinator._tranfile.keys()
            or self.entity_description.key
            in self.coordinator._tranfile[FIELD_DAYPART].keys()
        ):
            if self.forecast_day is not None:
                if self.entity_description.feature == FEATURE_FORECAST_DAYPART:
                    if (
                        ...
                        return (
                            self.coordinator._tranfile[FIELD_DAYPART][
                ...
                return (
                    self.coordinator._tranfile[self.entity_description.key]
                    + " "
                    + self.coordinator.data[FIELD_FORECAST_DAYOFWEEK][
                        self.forecast_day
                    ]
                )
