...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    verify_ssl = (
        entry.data[CONF_VERIFY_SSL] if CONF_VERIFY_SSL in entry.data else True
    )
    ...
    firmware, status = await hass.async_add_executor_job(
...
def register_services(
    hass: HomeAssistant, coord: TPLinkRouterCoordinator
) -> None:
    ...
    async def send_sms_service(service: ServiceCall) -> None:
        ...
        if device is None:
            _LOGGER.error(
                "TplinkRouter Integration Exception - device was not found"
            )
            return
        coordinator = None
        for key in device.config_entries:
            ...
            if not entry:
                continue
            if entry.domain != DOMAIN or not issubclass(
                hass.data[DOMAIN][key].router.__class__, TPLinkMRClient
            ):
                continue
        ...
        if coordinator is None:
            _LOGGER.error(
                "TplinkRouter Integration Exception - This device cannot send SMS"
            )
            return

        def callback():
            coord.router.send_sms(
                service.data.get("number"), service.data.get("text")
            )

        await hass.async_add_executor_job(
            TPLinkRouterCoordinator.request, coord.router, callback
        )
