...
def parse_sensirion(self, data: bytes, complete_local_name: str, mac: bytes):
    ...
    advertisementType0 = data[
        1
    ]  # redundant (also encoded in body - see below)
    ...
    if advertisementType == 0:
        ...
        _LOGGER.debug(
            "Advertisement Type %s not supported ", advertisementType
        )
        return None
...
def _parse_data_type(advSampleType, byte_data):
    if advSampleType == 3:
        ...
        return {"co2": _decodeSimple(byte_data[0:2])}
    else:
        _LOGGER.debug(
            "Advertisement SampleType %s not supported", advSampleType
        )
...
def _decodeTemperatureV1(byte_data):
    ...
    return round(
        (int.from_bytes(byte_data, byteorder="little") / 65535) * 175 - 45, 2
    )
...
def _decodeHumidityV1(byte_data):
    ...
    return round(
        (int.from_bytes(byte_data, byteorder="little") / 65535) * 100, 2
    )
...
def _decodeHumidityV2(byte_data):
    ...
    return round(
        ((int.from_bytes(byte_data, byteorder="little") * 125 / 65535) - 6), 2
    )
...
def _decodePM2p5V1(byte_data):
    ...
    return round(
        (int.from_bytes(byte_data, byteorder="little") / 65535) * 1000, 2
    )
