...
def parse_amazfit(
    self, service_data: str | None, man_spec_data: str, mac: bytes
):
    ...
    if service_data_length == 24:
        ...
        (impedance, weight, unk_1, unk_2, pulse) = unpack(
            "<5xHHBHB1x6x", xvalue
        )
        ...
        if heart_rate != 0xFF:
            result.update({"heart rate": heart_rate})
        if service_data_length == 8:
            (steps,) = unpack("I", service_data[4:])
            result.update({"steps": steps})
    else:
