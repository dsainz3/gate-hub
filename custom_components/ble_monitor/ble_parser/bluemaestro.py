...
def parse_bluemaestro(self, data: bytes, mac: bytes):
    ...
    if msg_length == 18 and device_id in [0x16, 0x17]:
        ...
        (batt, time_interval, log_cnt, temp, humi, dew_point, mode) = unpack(
            "!BhhhHhH", msg
        )
        result = {
            "temperature": temp / 10,
        ...
        # pylint: disable=unused-variable
        (batt, time_interval, log_cnt, temp, humi, press, mode) = unpack(
            "!BhhhHhH", msg
        )
        result = {
            "temperature": temp / 10,
        ...
        device_type = "Pebble"
        # pylint: disable=unused-variable
        (temp_1, temp_2, temp_3, humi, press) = unpack("<hhhBH", msg[0:9])
