...
class TestIBeacon:
    ...
    def test_ibeacon_tracker(self):
        ...
        ble_parser = BleParser(
            tracker_whitelist=[
                bytearray.fromhex("e2c56db5dffb48d2b060d0f5a71096e0")
            ]
        )
        sensor_msg, tracker_msg = ble_parser.parse_raw_data(data)
        ...
        assert (
            str(UUID(tracker_msg["uuid"]))
            == "e2c56db5-dffb-48d2-b060-d0f5a71096e0"
        )
        assert tracker_msg["uuid"] == "e2c56db5dffb48d2b060d0f5a71096e0"
        assert (
            tracker_msg["tracker_id"]
            == b"\xe2\xc5m\xb5\xdf\xfbH\xd2\xb0`\xd0\xf5\xa7\x10\x96\xe0"
        )
    ...
    def test_ibeacon_iphone_ble_packet_var_2(self):
        """Test iBeacon parser incorrect iphone ble packet"""
        data_string = (
            "043E1D02010001433EA2C96B6A1102011A020A0C0AFF4C001005441CBD08C5BB"
        )
        data = bytes(bytearray.fromhex(data_string))
        # pylint: disable=unused-variable
        ble_parser = BleParser()
