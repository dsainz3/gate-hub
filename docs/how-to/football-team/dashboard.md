...
- Toggle `input_boolean.huskers_use_score_delay` to hold new scoreboard values for ~30 seconds before the dashboards update. The buffer automation copies values into the manual helpers after the delay, and its parallel queue means rapid ESPN clock ticks stay in line instead of cancelling the hold. The sync automation refreshes everything on restart or toggle changes, and `automation.huskers_scoreboard_refresh_live` keeps the ESPN scoreboard REST sensor updating every 30 seconds so the delay has timely data even if TeamTracker stutters.
...
- **Automation trio**: `automation.huskers_score_delay_buffer` runs in parallel mode (`max: 120`) and waits 30 seconds after TeamTracker posts a new score/quarter/clock before copying the values into the manual helpers. If TeamTracker data is unavailable during a run, it now falls back to the ESPN auto scoreboard sensors so the helpers still refresh. `automation.huskers_score_delay_sync` re-aligns the helpers at startup or when toggling the buffer with the same fallback logic, and `automation.huskers_scoreboard_refresh_live` calls `homeassistant.update_entity` on `sensor.espn_cfb_scoreboard` every 30 seconds while the buffer is enabled or the game is live so the fallback feed stays within the expected delay window.
